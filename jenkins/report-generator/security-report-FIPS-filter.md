### Two scanners → one HTML report generator.

Reads `Trivy JSON (image/OS CVEs)` or `Syft SBOM + Grype JSON (component-level CVEs)`, auto-detects which, renders both into same styled searchable/sortable HTML page. 
**Filters out Oracle Linux FIPS-track false positives (epoch-10 package mismatches) from Grype results before rendering.**

```python
#!/usr/bin/env python3
"""
Author      : Sagar Malla
Description : Unified security report generator. Combines the Trivy
              JSON handler and the Syft/Grype SBOM+vuln handler into
              one file, sharing a single HTML/CSS/JS template so both
              report types look and behave identically.

              Auto-detects input type:
                - Trivy JSON        -> has top-level "Results" key
                - Grype JSON        -> has top-level "matches" key
                                        (requires a companion sbom.json
                                        from syft in the same directory,
                                        or passed explicitly)

              Usage:
                python3 security-report.py trivy-image-report.json
                python3 security-report.py trivy-image-report.json output.html

                python3 security-report.py vuln-report.json
                python3 security-report.py sbom.json vuln-report.json
                python3 security-report.py sbom.json vuln-report.json output.html

                # any invocation:
                python3 security-report.py <input(s)> [output.html] \
                    --pipeline-name X --build-number 12 --branch main
"""
...
def build_row_html(v):
    """Shared row renderer — same 7-column shape (Package/Version/Vuln/
    Severity/Description/Fix/Locations) used by both Trivy and Grype+SBOM
    inputs, so both report types share one table layout."""
...
def build_rows(vuln_mapping):
    rows = [build_row_html(v) for v in vuln_mapping]
...
# ── HTML Template Engine (shared by both report types) ───────────────────────

def build_html(title, subtitle, header_meta, extra_stat_label, extra_stat_value,
                vuln_mapping, no_data_msg, search_placeholder, pipeline_info=None):
    now = datetime.now()
    rows = build_rows(vuln_mapping)
    stat_cards = build_stat_cards(vuln_mapping, extra_cards=[(extra_stat_label, extra_stat_value, SVG_SHIELD)])
...
def handle_trivy(json_path, pipeline_info=None):
    ...
    return build_html(
        ...
        extra_stat_label="Targets Scanned",
        extra_stat_value=len(targets),
...
# ── Syft SBOM + Grype vuln-report handler ────────────────────────────────────

def handle_sbom(sbom_path, vuln_path, pipeline_info=None):
    ...
    for match in vuln_report.get("matches", []):
        ...
        if f"{name}:{version}" not in components:
            continue

        # Oracle Linux FIPS-variant packages use epoch 10 (e.g. "10:3.5.1-7...")
        # for the *same* fix already shipped on the normal (epoch 1) track.
        # Grype matches against these FIPS advisories even when the installed
        # non-FIPS package is already current -> false positive. Skip them.
        fix_versions = vuln.get("fix", {}).get("versions", []) or []
        if any(fv.startswith("10:") for fv in fix_versions):
            fips_filtered += 1
            continue
    ...
    header_meta = (
        f'<span>{SVG_CALENDAR} {datetime.now().strftime("%Y-%m-%d %H:%M")}</span>'
        f'<span>{SVG_PACKAGE} {len(components)} components scanned</span>'
    )
    if fips_filtered:
        header_meta += f'<span>{fips_filtered} FIPS-track false positive(s) filtered</span>'

    return build_html(
        title="SBOM Vulnerability Report",
        subtitle="Software Bill of Materials Security Analysis",
        header_meta=header_meta,
        extra_stat_label="Components",
        extra_stat_value=len(components),
        vuln_mapping=vuln_mapping,
...
def _detect_format(path):
    """Peek a JSON file's top-level keys to identify Trivy vs Grype output."""
    try:
        with open(path) as f:
            peek = json.load(f)
    except (json.JSONDecodeError, OSError):
        return None
    if isinstance(peek, dict) and "Results" in peek:
        return "trivy"
    if isinstance(peek, dict) and "matches" in peek:
        return "grype"
    return None
...
def main():
    parser = argparse.ArgumentParser(
        description="Unified Trivy / Syft+Grype security report generator."
    )
    parser.add_argument("files", nargs="+", help="Input JSON file(s), optionally followed by an output .html path")
    ...
    if parsed.pipeline_name:
        pipeline_info["name"] = parsed.pipeline_name
    if parsed.build_number:
        pipeline_info["build_number"] = parsed.build_number
    if parsed.branch:
        pipeline_info["branch"] = parsed.branch

    # Split trailing .html arg (output path) from the JSON inputs
    args = list(parsed.files)
    output = None
    if args and (args[-1].endswith(".html") or args[-1].endswith(".htm")):
        output = args.pop()

    if len(args) == 0:
        print("No input JSON file(s) provided.", file=sys.stderr)
        sys.exit(1)
    ...
    if len(args) == 1:
        ...
        if fmt == "trivy":
            html = handle_trivy(path, pipeline_info=pipeline_info)
            out = output or "trivy-report.html"
        elif fmt == "grype":
            # single grype vuln-report.json — look for a companion sbom.json
            p = Path(path)
            sbom_candidates = [p.parent / "sbom.json"]
            found = next((c for c in sbom_candidates if c.exists()), None)
            if not found:
                print(f"SBOM file not found for grype report. Tried: {sbom_candidates}", file=sys.stderr)
                sys.exit(1)
            html = handle_sbom(str(found), path, pipeline_info=pipeline_info)
            out = output or "vuln-report.html"
        else:
            print(f"Could not detect format (expected Trivy 'Results' or Grype 'matches') in: {path}", file=sys.stderr)
            sys.exit(1)

    else:
        # Two JSON inputs: sbom.json + vuln-report.json (order-tolerant)
        a, b = args[0], args[1]
        for p in (a, b):
            if not Path(p).exists():
                print(f"Input file not found: {p}", file=sys.stderr)
                sys.exit(1)
        fmt_a, fmt_b = _detect_format(a), _detect_format(b)

        if fmt_a == "grype" or fmt_b == "grype":
            sbom_path, vuln_path = (a, b) if fmt_b == "grype" else (b, a)
            html = handle_sbom(sbom_path, vuln_path, pipeline_info=pipeline_info)
            out = output or "vuln-report.html"
        elif fmt_a == "trivy":
            html = handle_trivy(a, pipeline_info=pipeline_info)
            out = output or "trivy-report.html"
        elif fmt_b == "trivy":
            html = handle_trivy(b, pipeline_info=pipeline_info)
            out = output or "trivy-report.html"
        else:
            print("Could not detect format for either input file.", file=sys.stderr)
            sys.exit(1)
...
```groovy
# Trivy
python3 /opt/security-report.py trivy-image-report.json
python3 /opt/security-report.py trivy-image-report.json trivy-report.html

# Grype + Syft SBOM (order doesn't matter)
python3 /opt/security-report.py sbom.json vuln-report.json vuln-report.html
python3 /opt/security-report.py vuln-report.json sbom.json vuln-report.html

# Single grype file, sbom.json auto-found in same dir
python3 /opt/security-report.py vuln-report.json

# with pipeline metadata
python3 /opt/security-report.py trivy-image-report.json trivy-report.html \
    --pipeline-name "$JOB_BASE_NAME" --build-number "$BUILD_NUMBER" --branch main
```
