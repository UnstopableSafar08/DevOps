...
echo "[3/5] Creating symlink /usr/local/java → ${JAVA_HOME_PATH}"
ln -sfn "$JAVA_HOME_PATH" /usr/local/java
...
# Heap: 6 GB of 16 GB total RAM
# Rule: never exceed 50% of RAM for JVM heap
# Remaining 10 GB feeds OS page cache — Kafka reads depend on it
KAFKA_HEAP_OPTS=-Xms6g -Xmx6g

# G1GC — best choice for Kafka's large heap with mixed allocation patterns
# MaxGCPauseMillis=20 — target pause goal (not guaranteed, G1 tries to meet it)
# IHOP=35 — start concurrent GC at 35% heap occupancy, prevents full GC
# G1HeapRegionSize=16m — large regions reduce GC overhead for big allocations
...
# GC log — rotate 5 files × 100 MB each
...
Environment="JAVA_HOME=/usr/lib/jvm/bellsoft-java21-amd64"
